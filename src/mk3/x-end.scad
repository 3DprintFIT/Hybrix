    ...
    rotate([90,0,-15,])
        translate ([-1.5, 8, 26])
            linear_extrude(height = 0.2)
                polygon(points=[[-2,0],[0,5],[8,0]]); 

    rotate([90,0,-50,])
        translate ([8.5, 8, 1.4])
            linear_extrude(height = 0.2)
                polygon(points=[[0,0],[0,5],[12,0]]); 
}
...
module x_end_holes() {
    ...
    // Top pushfit rod
    translate(v=[-15,-41.5,rod_distance+6])
        rotate(a=[-90,0,0])
            pushfit_rod(7.8,50);
    ...
    // Hole for the nut
    translate(v=[0,-17, -1])
        cylinder(h = 14.51, r = 5.5, $fn = 60);

    // Screw holes for TR nut
    translate(v=[0,-17, 0])
        rotate([0, 0, -135])
            translate([0, 8, -4])   
                cylinder(h = 19, r = 1.65, $fn=50);
    
    translate(v=[0,-17, 0])
        rotate([0, 0, -135])
            translate([0, -8, -4])  
                cylinder(h = 20, r = 1.65, $fn=50);
...
module pushfit_rod(diameter,length) {
    ...
    difference() {
        translate(v=[0,-diameter/2.85,length/2])
            rotate([0,0,45])
                cube(size = [diameter/2,diameter/2,length], center = true);
        
        translate(v=[0,-diameter/4-diameter/2-0.4,length/2])
            rotate([0,0,0])
                cube(size = [diameter,diameter/2,length], center = true);
    }
}
